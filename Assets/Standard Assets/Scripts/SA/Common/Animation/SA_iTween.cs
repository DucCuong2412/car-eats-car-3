...
namespace SA.Common.Animation
{
	public class SA_iTween : MonoBehaviour
	{
		...
		public static GameObject CameraFadeAdd(Texture2D texture, int depth)
		{
			...
			cameraFade.AddComponent<RawImage>();
			cameraFade.GetComponent<RawImage>().texture = texture;
			cameraFade.GetComponent<RawImage>().color = new Color(0.5f, 0.5f, 0.5f, 0f);
			return cameraFade;
		}
		...
		public static void Resume(GameObject target)
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				SA_iTween sA_iTween = (SA_iTween)array[i];
		...
		{
			Component[] components = GetComponents<SA_iTween>();
			Component[] array = components;
			int num = 0;
			SA_iTween sA_iTween;
			while (true)
			{
				...
				sA_iTween = (SA_iTween)array[num];
				if (sA_iTween.type == "value")
				{
					return;
				}
				if (sA_iTween.isRunning && sA_iTween.type == type)
				{
					if (sA_iTween.method != method)
					{
						return;
					}
					if (sA_iTween.tweenArguments.Count != tweenArguments.Count)
					{
						break;
					}
					...
					try
					{
						while (enumerator.MoveNext())
						{
							...
							if (!sA_iTween.tweenArguments[dictionaryEntry.Key].Equals(tweenArguments[dictionaryEntry.Key]) && (string)dictionaryEntry.Key != "id")
							{
								sA_iTween.Dispose();
								return;
							}
