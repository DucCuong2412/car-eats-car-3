...
public class NativeIOSActionsExample : BaseIOSFeaturePreview
{
	...
	private void Awake()
	{
		...
		IOSDateTimePicker instance = Singleton<IOSDateTimePicker>.Instance;
		instance.OnDateChanged = (Action<DateTime>)Delegate.Combine(instance.OnDateChanged, new Action<DateTime>(OnDateChanged));
		IOSDateTimePicker instance2 = Singleton<IOSDateTimePicker>.Instance;
	...
	private void OnGUI()
	{
		...
		{
			Singleton<IOSDateTimePicker>.Instance.Show(IOSDateTimePickerMode.Time);
		}
		StartX += XButtonStep;
		if (GUI.Button(new Rect(StartX, StartY, buttonWidth, buttonHeight), "Date"))
		{
			Singleton<IOSDateTimePicker>.Instance.Show(IOSDateTimePickerMode.Date);
		}
		StartX += XButtonStep;
		if (GUI.Button(new Rect(StartX, StartY, buttonWidth, buttonHeight), "Date And Time"))
		{
			Singleton<IOSDateTimePicker>.Instance.Show(IOSDateTimePickerMode.DateAndTime);
		}
		StartX += XButtonStep;
		if (GUI.Button(new Rect(StartX, StartY, buttonWidth, buttonHeight), "Countdown Timer"))
		{
			Singleton<IOSDateTimePicker>.Instance.Show(IOSDateTimePickerMode.CountdownTimer);
		}
		StartX = XStartPos;
		StartY += YButtonStep;
		if (GUI.Button(new Rect(StartX, StartY, buttonWidth, buttonHeight), "Set Date Without UI"))
		{
			Singleton<IOSDateTimePicker>.Instance.Show(IOSDateTimePickerMode.Date, time);
		}
		StartX = XStartPos;
		StartY += YButtonStep;
		StartY += YLableStep;
