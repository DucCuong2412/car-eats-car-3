...
public static class tk2dTextGeomGen
{
	...
	public static int SetTextMeshGeom(Vector3[] pos, Vector2[] uv, Vector2[] uv2, Color32[] color, int offset, GeomData geomData)
	{
		...
		Vector2 meshDimensionsForString = GetMeshDimensionsForString(geomData.formattedText, geomData);
		float yAnchorForHeight = GetYAnchorForHeight(meshDimensionsForString.y, geomData);
		float num = 0f;
		float num2 = 0f;
		int num3 = 0;
		int num4 = 0;
		for (int i = 0; i < formattedText.Length && num3 < textMeshData.maxChars; i++)
		{
			...
			{
				uv2[offset + num3 * 4] = tk2dFontChar.gradientUv[0] + new Vector2(meshGradientTexU, 0f);
				uv2[offset + num3 * 4 + 1] = tk2dFontChar.gradientUv[1] + new Vector2(meshGradientTexU, 0f);
				uv2[offset + num3 * 4 + 2] = tk2dFontChar.gradientUv[2] + new Vector2(meshGradientTexU, 0f);
				uv2[offset + num3 * 4 + 3] = tk2dFontChar.gradientUv[3] + new Vector2(meshGradientTexU, 0f);
			}
			if (fontInst.isPacked)
			{
				Color32 color2 = channelSelectColors[tk2dFontChar.channel];
