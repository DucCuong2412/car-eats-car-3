...
public class AIActivator : MonoBehaviour
{
	public class AIObject
	{
		...
		public Car2DControlerForBombCar aiControllerForBombCar;

		public AIObjectNode node;
		...
		public bool enabled
		{
			...
			set
			{
				...
				{
					aiControllerForBombCar.isKinematic = !value;
				}
			}
		}
		...
		public void SetEnabled(bool enabled)
		{
			...
			{
				aiControllerForBombCar.Enabled = enabled;
			}
			...
			Vector2 origin = (!aiController) ? ((Vector2)(aiControllerForBombCar.SuspensionModule.rearwheel.transform.position - 10f * Vector3.up)) : ((Vector2)(aiController.SuspensionModule.rearwheel.transform.position - 10f * Vector3.up));
			RaycastHit2D raycastHit2D = Physics2D.Raycast(origin, -Vector2.up, float.MaxValue, layerCast);
			Vector2 origin2 = (!aiController) ? ((Vector2)(aiControllerForBombCar.SuspensionModule.frontwheel.transform.position - 10f * Vector3.up)) : ((Vector2)(aiController.SuspensionModule.frontwheel.transform.position - 10f * Vector3.up));
	...
	private Stack<AIObjectNode> rStack = new Stack<AIObjectNode>();

	private Stack<AIObject> stack = new Stack<AIObject>();

	private List<AIObjectNode> needToActivateList = new List<AIObjectNode>();
	...
	private static string strEnemy = "Enemy ";

	private static string strClone = "(Clone)";

	private static string strEmpty = string.Empty;

	private float distanceToActivate => LevelBuilder.aiAD;

	public void AddCarToList(GameObject obj)
	{
		AIObject aIObject = new AIObject();
		aIObject.gameObject = obj;
		aIObject.transform = obj.transform;
		aIObject.aiControllerForBombCar = obj.GetComponent<Car2DControlerForBombCar>();
	...
	public void CreateStack(Transform follower, int pack, int stackLength = 10)
	{
		...
		GameObject original = Progress.shop.Undeground2 ? (Resources.Load("enemy_underground_02") as GameObject) : (Progress.shop.TestFor9 ? (Resources.Load("enemy_underground_01") as GameObject) : ((!Progress.shop.EsterLevelPlay) ? (Resources.Load(strEnemy + pack.ToString()) as GameObject) : (Resources.Load("Enemy_easter") as GameObject)));
		for (int i = 0; i < stackLength; i++)
		{
			GameObject gameObject = UnityEngine.Object.Instantiate(original);
			gameObject.transform.parent = base.transform;
			gameObject.transform.localPosition = new Vector3(0f, 0f, 120f);
	...
	public void Restart(List<AIObjectNode> list)
	{
		isStarted = false;
		rStack.Clear();
		...
		{
			if ((bool)activeList[0].aiController)
			{
	...
	private void Update()
	{
		...
		{
			AIObjectNode aIObjectNode = needToActivateList[num];
			if (aIObjectNode.isAhead)
			{
				float x2 = aIObjectNode.position.x;
				Vector3 position2 = followTransform.position;
				if (x2 < position2.x - distanceToActivate)
				{
					needToActivateList.RemoveAt(num);
					SpawnCar(aIObjectNode);
				}
			}
			else
			{
				float x3 = aIObjectNode.position.x;
		...
		for (int num2 = activeList.Count - 1; num2 >= 0; num2--)
		{
			...
			{
				if ((bool)activeList[num2].aiController)
				{
			...
			else if (!activeList[num2].enabled)
			{
				...
				{
					if (activeList[num2].aiController == null)
					{
						activeList[num2].position += Vector2.right * Time.deltaTime * 50f;
					}
					else
					{
				...
				else
				{
					...
					if (x5 > position7.x + 1.5f * distanceToActivate)
					{
						if (activeList[num2].aiController == null)
						{
							activeList[num2].position -= Vector2.right * Time.deltaTime * 50f;
			...
			else
			{
				...
				if (x6 < position9.x - 2f * distanceToActivate)
				{
				...
				else
				{
					...
					if (x7 > position11.x + 2f * distanceToActivate)
					{
						activeList[num2].SetEnabled(enabled: false);
					}
				}
	...
	private void SpawnCar(AIObjectNode node)
	{
		if (stack.Count == 0)
		{
