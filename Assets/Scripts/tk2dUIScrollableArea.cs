...
public class tk2dUIScrollableArea : MonoBehaviour
{
	...
	private void BackgroundOverUpdate()
	{
		...
		if (scrollBar != null)
		{
			...
			{
				Vector3 contentContainerOffset2 = ContentContainerOffset;
				scrollPercentWithoutEvent = contentContainerOffset2.y / (contentLength - visibleAreaLength);
			}
	...
	{
		Vector3 a = base.transform.InverseTransformPoint(CalculateClickWorldPos(backgroundUIItem));
		Vector3 b = a - swipeScrollingPressDownStartLocalPos;
		b.x *= -1f;
		float f = 0f;
		if (scrollAxes == Axes.XAxis)
		{
			f = b.x;
			swipeCurrVelocity = 0f - (a.x - swipePrevScrollingContentPressLocalPos.x);
		}
		else if (scrollAxes == Axes.YAxis)
		{
			f = b.y;
			swipeCurrVelocity = a.y - swipePrevScrollingContentPressLocalPos.y;
		}
		...
		if (isSwipeScrollingInProgress)
		{
			Vector3 vector = swipeScrollingContentStartLocalPos + b;
			Vector3 contentContainerOffset = ContentContainerOffset;
			vector.z = contentContainerOffset.z;
			if (scrollAxes == Axes.XAxis)
			{
				Vector3 contentContainerOffset2 = ContentContainerOffset;
				vector.y = contentContainerOffset2.y;
			}
			else if (scrollAxes == Axes.YAxis)
			{
				Vector3 contentContainerOffset3 = ContentContainerOffset;
				vector.x = contentContainerOffset3.x;
			}
			Vector3 contentContainerOffset4 = ContentContainerOffset;
			vector.z = contentContainerOffset4.z;
			swipeScrollingContentDestLocalPos = vector;
			swipePrevScrollingContentPressLocalPos = a;
		}
	}
	...
	private Vector3 CalculateClickWorldPos(tk2dUIItem btn)
	{
		...
		float x = position.x;
		float y = position.y;
		Vector3 position2 = btn.transform.position;
		float z = position2.z;
		Vector3 position3 = uICameraForControl.transform.position;
		Vector3 result = camera.ScreenToWorldPoint(new Vector3(x, y, z - position3.z));
		Vector3 position4 = btn.transform.position;
		result.z = position4.z;
		return result;
	}
