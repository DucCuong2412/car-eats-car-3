...
public abstract class tk2dBaseSprite : MonoBehaviour, ISpriteCollectionForceBuild
{
	...
	public static T AddComponent<T>(GameObject go, tk2dSpriteCollectionData spriteCollection, string spriteName) where T : tk2dBaseSprite
	{
		...
		if (spriteIdByName == -1)
		{
			UnityEngine.Debug.LogError($"Unable to find sprite named {spriteName} in sprite collection {spriteCollection.spriteCollectionName}");
			return (T)null;
		}
	...
	{
		tk2dSpriteDefinition tk2dSpriteDefinition = collectionInst.spriteDefinitions[spriteId];
	...
	{
		InitInstance();
		tk2dSpriteDefinition tk2dSpriteDefinition = collectionInst.spriteDefinitions[_spriteId];
		return new Bounds(new Vector3(tk2dSpriteDefinition.boundsData[0].x * _scale.x, tk2dSpriteDefinition.boundsData[0].y * _scale.y, tk2dSpriteDefinition.boundsData[0].z * _scale.z), new Vector3(tk2dSpriteDefinition.boundsData[1].x * Mathf.Abs(_scale.x), tk2dSpriteDefinition.boundsData[1].y * Mathf.Abs(_scale.y), tk2dSpriteDefinition.boundsData[1].z * Mathf.Abs(_scale.z)));
	}
		...
		if (tk2dSpriteDefinition.physicsEngine == tk2dSpriteDefinition.PhysicsEngine.Physics3D)
		{
			...
			if (boxCollider != null)
			{
				if (tk2dSpriteDefinition.colliderType == tk2dSpriteDefinition.ColliderType.Box)
				{
					...
					boxCollider.size = new Vector3(2f * tk2dSpriteDefinition.colliderVertices[1].x * _scale.x, 2f * tk2dSpriteDefinition.colliderVertices[1].y * _scale.y, 2f * tk2dSpriteDefinition.colliderVertices[1].z * _scale.z);
				}
			...
			else if (tk2dSpriteDefinition.colliderType == tk2dSpriteDefinition.ColliderType.Mesh)
			{
				...
				for (int j = 0; j < num; j++)
				{
					...
					if (_scale.x != 1f || _scale.y != 1f)
					{
						...
						{
							array[k] = Vector2.Scale(points[k], _scale);
						}
						polygonCollider2D[j].points = array;
					}
					else
					{
						polygonCollider2D[j].points = tk2dSpriteDefinition.polygonCollider2D[j].points;
					}
				}
				...
				for (int n = 0; n < num2; n++)
				{
					...
					if (_scale.x != 1f || _scale.y != 1f)
					{
						...
						{
							array2[num3] = Vector2.Scale(points2[num3], _scale);
						}
						edgeCollider2D[n].points = array2;
					}
					else
					{
						edgeCollider2D[n].points = tk2dSpriteDefinition.edgeCollider2D[n].points;
					}
				}
	...
	public static GameObject CreateFromTexture<T>(Texture texture, tk2dSpriteCollectionSize size, Rect region, Vector2 anchor) where T : tk2dBaseSprite
	{
		...
		AddComponent<T>(gameObject, tk2dSpriteCollectionData, 0);
		return gameObject;
	}
}
