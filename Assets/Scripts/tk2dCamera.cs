...
public class tk2dCamera : MonoBehaviour
{
	...
	public static tk2dCamera CameraForLayer(int layer)
	{
		...
		{
			tk2dCamera tk2dCamera = allCameras[i];
	...
	{
		tk2dCameraSettings tk2dCameraSettings = SettingsRoot.CameraSettings;
		switch (tk2dCameraSettings.projection)
		{
		case tk2dCameraSettings.ProjectionType.Orthographic:
			...
			return 2f * tk2dCameraSettings.orthographicSize / (float)SettingsRoot.nativeResolutionHeight;
	...
	private void Upgrade()
	{
		...
		if (version == 0)
		{
			...
			{
				tk2dCameraResolutionOverride.Upgrade(version);
			}
		...
		UnityEngine.Debug.Log("tk2dCamera '" + base.name + "' - Upgraded from version " + version.ToString());
	...
	public void UpdateCameraMatrix()
	{
		...
		if (tk2dCameraSettings.projection == tk2dCameraSettings.ProjectionType.Perspective)
		{
			...
			float num = Mathf.Min(179.9f, tk2dCameraSettings.fieldOfView / Mathf.Max(0.001f, ZoomFactor));
