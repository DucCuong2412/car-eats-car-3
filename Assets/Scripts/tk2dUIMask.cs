...
public class tk2dUIMask : MonoBehaviour
{
	...
	private void OnDrawGizmosSelected()
	{
		...
		if (sharedMesh != null)
		{
			...
			Vector3 center3 = new Vector3(x, center2.y, num * 0.5f);
			Vector3 extents = bounds.extents;
			float x2 = extents.x * 2f;
			Vector3 extents2 = bounds.extents;
			Vector3 vector = new Vector3(x2, extents2.y * 2f, Mathf.Abs(num));
			Gizmos.DrawCube(center3, vector);
			Gizmos.color = new Color32(22, 145, byte.MaxValue, byte.MaxValue);
			Gizmos.DrawWireCube(center3, vector);
		}
	}
	...
	public void Build()
	{
		...
		if (createBoxCollider)
		{
			...
			BoxCollider thisBoxCollider = ThisBoxCollider;
			Vector3 center = bounds.center;
			float x = center.x;
			Vector3 center2 = bounds.center;
			thisBoxCollider.center = new Vector3(x, center2.y, 0f - depth);
			BoxCollider thisBoxCollider2 = ThisBoxCollider;
			Vector3 vector = bounds.size;
			float x2 = vector.x;
			Vector3 vector2 = bounds.size;
			thisBoxCollider2.size = new Vector3(x2, vector2.y, 0.0002f);
		}
	...
	public void ReshapeBounds(Vector3 dMin, Vector3 dMax)
	{
		...
		Vector3 b = new Vector3((!Mathf.Approximately(vector.x, 0f)) ? (zero.x * vector2.x / vector.x) : 0f, (!Mathf.Approximately(vector.y, 0f)) ? (zero.y * vector2.y / vector.y) : 0f);
