...
public class GroundActivation : MonoBehaviour
{
	...
	private static GroundActivation p_instance;
	...
	public void CreateStack(Transform follower, int pack, int stackLength = 10)
	{
		...
		{
			groundInstance = (Resources.Load("Ground_10") as GameObject).GetComponent<EasyHill2DNode>();
		}
		else if (Progress.shop.TestFor9)
		{
			groundInstance = (Resources.Load("Ground_9") as GameObject).GetComponent<EasyHill2DNode>();
		}
		...
		{
			groundInstance = (Resources.Load("Ground_4_1") as GameObject).GetComponent<EasyHill2DNode>();
		}
	...
	private void Update()
	{
		...
		float groundAD = LevelBuilder.groundAD;
		if (rStack.Count > 0)
		{
			do
			{
		...
		for (int i = 0; i < num2; i++)
		{
			...
			float num4 = position5.x + ground.node.p1.x;
			Vector3 position6 = followTransform.position;
			if (num4 > position6.x + groundAD)
			{
				RemoveObject(ground);
	...
	private Ground SpawnObject(GroundObjectNode gNode)
	{
		...
		ground.node.transform.rotation = gNode.Rotation;
		ground.node.p2 = gNode.P2;
		ground.node.p3 = gNode.P3;
		ground.node.p4 = gNode.P4;
		ground.node.gameObject.layer = gNode.layer;
