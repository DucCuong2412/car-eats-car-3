...
public class tk2dTileMap : MonoBehaviour, ISpriteCollectionForceBuild
{
	...
	public void Build(BuildFlags buildFlags)
	{
		...
		bool flag = (buildFlags & BuildFlags.ForceBuild) != BuildFlags.Default;
	...
	public Vector3 GetTilePosition(int x, int y)
	{
		...
		Vector3 v2 = new Vector3(((float)x + (((y & 1) != 0) ? 0.5f : 0f)) * data.tileSize.x + data.tileOrigin.x, (float)y * data.tileSize.y + data.tileOrigin.y, 0f);
