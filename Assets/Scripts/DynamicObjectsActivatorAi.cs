...
public class DynamicObjectsActivatorAi : MonoBehaviour
{
	...
	public List<DynamicNode> activeObjects = new List<DynamicNode>();
	...
	public void Update()
	{
		...
		if (waitingList.Count > 0)
		{
			if (rStack.Count > 0)
			{
				...
				if (x2 < position2.x + distanceToActivate * 0.7f || waitingList[waitingList.Count - 1].position.x + distanceToActivate < rStack.Peek().position.x)
				{
		...
		for (int k = 0; k < activeObjects.Count; k++)
		{
			...
			dynamicNode.node.scale = dynamicNode.gameObject.transform.localScale;
