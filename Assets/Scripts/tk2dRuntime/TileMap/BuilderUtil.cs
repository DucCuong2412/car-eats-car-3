...
namespace tk2dRuntime.TileMap
{
	public static class BuilderUtil
	{
		...
		private static GameObject GetExistingTilePrefabInstance(tk2dTileMap tileMap, int tileX, int tileY, int tileLayer)
		{
			...
			{
				tileMap.GetTilePrefabsListItem(i, out int x, out int y, out int layer, out GameObject instance);
			...
			for (int i = 0; i < tileMap.partitionSizeY; i++)
			{
				...
				for (int j = 0; j < tileMap.partitionSizeX; j++)
				{
					...
					if (gameObject != null)
					{
						...
						Vector3 vector = new Vector3(tileSize.x * ((float)j + num), tileSize.y * (float)i, 0f);
						bool flag2 = false;
		...
		public static void HideTileMapPrefabs(tk2dTileMap tileMap)
		{
			...
			for (int i = 0; i < tileMap.Layers.Length; i++)
			{
				...
				for (int j = 0; j < layer.spriteChannel.chunks.Length; j++)
				{
					...
					for (int k = 0; k < childCount; k++)
					{
						...
						{
							tileMap.GetTilePrefabsListItem(l, out int _, out int _, out int _, out GameObject instance);
			...
			{
				tileMap.GetTilePrefabsListItem(m, out int x2, out int y2, out int layer3, out GameObject instance2);
				if (!array[m])
				{
		...
		public static void InvertRawTileFlag(ref int rawTile, tk2dTileFlags flag)
		{
			if (rawTile != -1)
			{
				rawTile = (((rawTile & (int)flag) != 0) ? (rawTile & (int)(~flag)) : (rawTile | (int)flag));
			}
		}
