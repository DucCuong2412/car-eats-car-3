...
namespace tk2dRuntime
{
	internal static class SpriteCollectionGenerator
	{
		...
		{
			return CreateFromTexture(texture, size, new string[1]
			{
				"Unnamed"
			}, new Rect[1]
			{
		...
		public static tk2dSpriteCollectionData CreateFromTexture(GameObject parentObject, Texture texture, tk2dSpriteCollectionSize size, Vector2 textureDimensions, string[] names, Rect[] regions, Rect[] trimRects, Vector2[] anchors, bool[] rotated)
		{
			...
			tk2dSpriteCollectionData.materials = new Material[1]
			{
				tk2dSpriteCollectionData.material
			};
			tk2dSpriteCollectionData.textures = new Texture[1]
			{
				texture
			};
		...
		private static tk2dSpriteDefinition CreateDefinitionForRegionInTexture(string name, Vector2 textureDimensions, float scale, Rect uvRegion, Rect trimRect, Vector2 anchor, bool rotated)
		{
			...
			Vector2 a = new Vector2(trimRect.x - anchor.x, 0f - trimRect.y + anchor.y);
			if (rotated)
			{
				a.y -= width;
			}
			a *= scale;
			Vector3 a2 = new Vector3((0f - anchor.x) * scale, anchor.y * scale, 0f);
			Vector3 vector4 = a2 + new Vector3(trimRect.width * scale, (0f - trimRect.height) * scale, 0f);
			Vector3 a3 = new Vector3(0f, (0f - height) * scale, 0f);
			Vector3 vector5 = a3 + new Vector3(width * scale, height * scale, 0f);
			if (rotated)
			{
				tk2dSpriteDefinition.positions = new Vector3[4]
				{
					new Vector3(0f - vector5.y + a.x, a3.x + a.y, 0f),
					new Vector3(0f - a3.y + a.x, a3.x + a.y, 0f),
					new Vector3(0f - vector5.y + a.x, vector5.x + a.y, 0f),
					new Vector3(0f - a3.y + a.x, vector5.x + a.y, 0f)
				};
			...
			tk2dSpriteDefinition.indices = new int[6]
			{
				0,
				3,
				1,
